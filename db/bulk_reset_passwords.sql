...
-- Step 1: Add requires_password_change column if not present
ALTER TABLE members ADD COLUMN IF NOT EXISTS requires_password_change BOOLEAN DEFAULT false;
...
-- Step 3: Set all general members password to Udaan@2026
-- and flag them to change password on first login
...
SELECT 
  ...
  COUNT(CASE WHEN member_id = 'UDAAN-000' THEN 1 END) as super_admins,
  COUNT(CASE WHEN requires_password_change = true THEN 1 END) as pending_password_changes
FROM members;
